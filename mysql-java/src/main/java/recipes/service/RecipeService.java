...
private String removeComments(String content) {
	...
	int commentPos = 0;//counter to loop through the content, comment to comment, keeping track of position
	
	while((commentPos = builder.indexOf("-- ", commentPos)) != -1){ //will return either a position or -1
		int eolPos = builder.indexOf("\n", commentPos +1);
		
		if(eolPos == -1) {
			builder.replace(commentPos, builder.length(), "");//at commentPos, take builder.length(), replace with nothing
		}else {
