...
//@formatter: off
		...
		String sql = ""
				/*
				 * will select unit name from ingredient table whether or not a unit exists will
				 * order by ingredient order
				 */
				+ "SELECT i.*, u.unit_name_singular, u.unit_name_plural " + "FROM " + INGREDIENT_TABLE + " i "
...
//@formatter: on
	...
	public Recipe insertRecipe(Recipe recipe) {// workflow: obtain connection, create prepared statement, set
		...
		try (Connection conn = DbConnection.getConnection()) {// try with resources
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				// look at this method later, sends a query to MySQL to get the last insert
				// id/autocommand value
	...
	public void addIngredientToRecipe(Ingredient ingredient) {
		...
		try (Connection conn = DbConnection.getConnection()) {
			...
			try {
				// getting the child rows and adding to it
	...
	public void addStepToRecipe(Step step) {
		...
		try (Connection conn = DbConnection.getConnection()) {
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				//because it's an insert, call execute update instead of result set
				stmt.executeUpdate();
				commitTransaction(conn);
	...
	public boolean modifyRecipeStep(Step step) {
		...
		try(Connection conn = DbConnection.getConnection()){
			...
			try(PreparedStatement stmt = conn.prepareStatement(sql)){
				...
				//execute update returns a value, the number of rows effected
	...
	public boolean deleteRecipe(Integer recipeId) {
		...
		try(Connection conn = DbConnection.getConnection()){
			...
			try(PreparedStatement stmt = conn.prepareStatement(sql)){
				...
				//if one row is affected, we'll know we've done our delete
